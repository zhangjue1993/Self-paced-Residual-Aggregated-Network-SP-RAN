...
class vgg16:
    def __init__(self, imgs, weights=None, sess=None):
        ...
        self.prediction = tf.argmax(self.fc3l, 1)
    ...
    def fc_layers(self):
        ...
        with tf.name_scope('fc2') as scope:
            # fc2w = tf.Variable(tf.truncated_normal([256, 256],
            #                                              dtype=tf.float32,
            #                                              stddev=1e-1), name='weights') # previous
            fc2w = tf.Variable(tf.truncated_normal([256, 256],
                                                         dtype=tf.float32,
                                                         stddev=1e-2), name='weights')
        ...
        with tf.name_scope('fc3') as scope:
            # fc3w = tf.Variable(tf.truncated_normal([256, 2],
            #                                              dtype=tf.float32,
            #                                              stddev=1e-1), name='weights') #previous
            fc3w = tf.Variable(tf.truncated_normal([256, 2],
                                                         dtype=tf.float32,
    ...
    def load_weights(self, weight_file, sess):
        ...
        npz = np.load(weight_file, encoding='latin1',allow_pickle = True,fix_imports=True).item()
        #
        params = []
        temp = sorted(npz.items())
        # print temp
        for num, val in enumerate(temp):
            ...
            sess.run(self.parameters[2*num].assign(W))
            sess.run(self.parameters[2*num+1].assign(b))